...
def _gen_sql(table_name,mappings):
    ...
    """
    pk = None
    sql = ['---genrating SQL for %s:' %table_name,'create table `%s` (' %table_name]
    for f in sorted(mappings.values(),lambda x,y:cmp(x.order,y_order)):
...
class Field(object):
    ...
    def __init__(self,**kw):
        ...
        self._default = kw.get('_default',None)
    ...
    def __str__(self):
        ...
        """
        a = ['<%s :%s,%s,default(%s),'%(self.__class__.__name__,self.name,self.ddl,self._default)]
...
class ModelMetaclass(type):    
    ...
    def __new__(cls,name,bases,attrs):
            ...
            if isinstance(v,Field):
                ...
                if v.primary_key:
                    ...
                    if v.updatable:
                        ...
                        v.updatable = False
                    if v.nullable:
                        logging.warning('NOTE:changec primary_key to non-nullable.')
                        v.nullable = False
                    primary_key = v
        ...
        attrs['_primary_key__'] = primary_key
